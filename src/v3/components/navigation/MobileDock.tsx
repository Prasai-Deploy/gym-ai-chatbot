      ...
      <div className="flex items-center justify-around w-full relative">
        {DOCK_ITEMS.map((item) => {
            ...
            return (
              <div key={item.id} className="relative -top-5 flex flex-col items-center">
                <button
                  ...
                  className="w-14 h-14 rounded-2xl bg-[#F97316] text-white flex items-center justify-center shadow-lg shadow-orange-500/40 hover:scale-105 active:scale-95 transition-all border border-orange-400/40"
          ...
          return (
            ...
            >
              <div className={`transition-all duration-200 ${isActive ? 'text-[#F97316] scale-110' : 'text-slate-500'}`}>
                {item.icon}
              </div>
              ...
              {isActive && (
                <div className="w-1.5 h-1.5 rounded-full bg-[#F97316] mt-0.5 shadow-sm shadow-orange-500" />
              )}
            </button>
          );
        })}
