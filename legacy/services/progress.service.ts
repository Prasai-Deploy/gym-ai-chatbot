 ...
 * Weekly progress and daily fitness stats via Supabase.
 */
import supabase from "../db.js";

export async function updateWeeklyProgress(userId: number, date: string, data: any) {
...
export async function updateDailyFitnessStats(userId: number, data: any) {
  const { data: existing } = await supabase
...
export async function getWeeklyChartData(userId: number) {
  ...
  const { data } = await supabase
    .from("weekly_progress")
    .select("date, workouts_completed, exercises_completed, calories_burned, workout_duration, hydration_completion, diet_completion")
    .eq("user_id", userId)
