 ...
 * AI-generated workout and diet plan management via Supabase.
 */
import supabase from "../db.js";
import { updateWeeklyProgress } from "./progress.service.js";

export async function saveAIWorkout(userId: number, plan: any, planId?: number) {
...
export async function saveAIDiet(userId: number, plan: any, planId?: number) {
...
export async function linkActivePlans(userId: number, workoutId?: number, dietId?: number) {
  ...
  const { data } = await supabase.from("user_fitness_plans").insert({
    user_id: userId,
    workout_plan_id: workoutId || null,
    diet_plan_id: dietId || null,
    active: 1,
  }).select("id").single();
  return data?.id;
}
