 ...
 * HTTP handlers for the progress dashboard API.
 *
 *  GET  /api/dashboard/:userId     → full dashboard data
 *  POST /api/progress/metrics       → log body weight / measurements
 */
...
export async function logMetricsHandler(
  req: Request,
  res: Response
): Promise<void> {
  const user = requireAuth(req, res);
