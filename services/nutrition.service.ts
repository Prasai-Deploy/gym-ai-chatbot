 ...
 */
export function calculateMacroGoals(profile: any): MacroGoals {
 ...
 */
export async function generateMealPlan(userId: number): Promise<any> {
...
Include specific Indian food items like Poha, Paneer, Roti, Dal, etc., based on the diet type.`;
  ...
  // Save to database
  await pool.execute(
    `INSERT INTO meal_plans (user_id, date, calories_target, meals)
     VALUES (?, ?, ?, ?)
 ...
 */
export async function logFoodIntake(userId: number, foodText: string): Promise<any> {
  const today = new Date().toISOString().split("T")[0];
...
If multiple items are mentioned, group them into one entry or return the total estimate.`;
  ...
  // Save to database
  await pool.execute(
    `INSERT INTO nutrition_logs (user_id, date, food_item, calories, protein, carbs, fats)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
 ...
 */
export async function getDailySummary(userId: number, date: string): Promise<any> {
