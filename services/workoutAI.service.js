 ...
 */
export function decideSplit(workoutDays, recentFocuses) {
    let sequence;
    if (workoutDays <= 1)
        return "Full Body";
    if (workoutDays === 2)
        return recentFocuses[0] === "Upper Body" ? "Lower Body" : "Upper Body";
    if (workoutDays === 3)
        sequence = SPLIT_3DAY;
    ...
    if (!lastFocus)
        return sequence[0]; // no history → start at beginning
    const lastIdx = sequence.findIndex((s) => s.toLowerCase() === lastFocus.toLowerCase());
    if (lastIdx === -1)
        return sequence[0]; // unknown focus → restart
 ...
 */
function applyProgressiveOverload(history) {
 ...
 */
export function buildWorkoutPrompt(profile, todayFocus, recentFocuses, historyMap) {
    ...
    for (const [name, history] of historyMap.entries()) {
        ...
        overloadLines.push(`  - ${name}: last used ${history.weight_used ?? 0} kg × ${history.reps_done ?? "?"} reps → suggest ${suggestedWeight} × ${suggestedReps} reps`);
    }
 ...
 */
export async function callWorkoutAI(prompt) {
    const raw = await callAI(prompt, "You are a professional personal trainer AI.", [], {
        temperature: 0.6,
        top_p: 0.8
    });
