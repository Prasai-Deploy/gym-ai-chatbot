...
export async function getProfile(userId) {
    const [rows] = await pool.execute("SELECT * FROM user_profiles WHERE user_id = ?", [userId.toString()]);
    return rows[0] ?? null;
}
/**
...
export async function upsertProfile(userId, data) {
    ...
    const values = [userId.toString(), ...fields.map((f) => filtered[f])];
    await pool.execute(`INSERT INTO user_profiles (${insertCols})
     VALUES (${placeholders})
