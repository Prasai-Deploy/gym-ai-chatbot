...
export async function createActivity(
  ...
  const [result] = await pool.execute(
    `INSERT INTO activity_logs (user_id, activity_type, activity_title, activity_description, metadata_json)
     VALUES (?, ?, ?, ?, ?)`,
    [userId, type, title, description || null, metadata ? JSON.stringify(metadata) : null]
  );

  const activityId = (result as ResultSetHeader).insertId;

  // Update tracking state
  await pool.execute(
    `INSERT INTO activity_tracking_state (user_id, latest_activity_id, unread_activity_count)
     VALUES (?, ?, 1)
     ON DUPLICATE KEY UPDATE 
       latest_activity_id = VALUES(latest_activity_id),
       unread_activity_count = unread_activity_count + 1`,
    [userId, activityId]
  );
...
export async function markActivitiesAsRead(userId: number) {
  await pool.execute(
    "UPDATE activity_tracking_state SET unread_activity_count = 0 WHERE user_id = ?",
    [userId]
  );
}
