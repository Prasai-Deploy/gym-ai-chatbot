 ...
 */
export async function saveAIWorkout(userId: number, plan: AIWorkoutPlan, planId?: string) {
  const [result] = await pool.execute(
    `INSERT INTO chatbot_generated_workouts 
 ...
 */
export async function saveAIDiet(userId: number, plan: AIDietPlan, planId?: string) {
  const [result] = await pool.execute(
    `INSERT INTO chatbot_generated_diets
 ...
 */
export async function getLatestActivePlan(userId: number) {
  ...
  if (plan) {
    if (plan.workout_exercises) plan.workout_exercises = JSON.parse(plan.workout_exercises);
    if (plan.diet_meals) plan.diet_meals = JSON.parse(plan.diet_meals);
  }
  return plan || null;
}
 ...
 */
export async function getPlanHistory(userId: number, limit = 10) {
  const [rows] = await pool.execute(
    `SELECT ufp.*, 
     ...
     LEFT JOIN chatbot_generated_diets cgd ON ufp.diet_plan_id = cgd.id
     WHERE ufp.user_id = ?
     ORDER BY ufp.created_at DESC LIMIT ?`,
    [userId, limit]
  );
  return rows;
}
 ...
 */
export async function logMeal(userId: number, date: string, meal: any) {
  const [result] = await pool.execute(
    `INSERT INTO user_meal_tracking (user_id, date, meal_type, food_item, calories, protein, carbs, fats)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      userId,
      date,
