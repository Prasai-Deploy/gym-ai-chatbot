...
export async function getWeightProgress(
  ...
  if (rows.length === 0) {
    const [profileRows] = await pool.execute(
      `SELECT weight_kg, updated_at FROM fitness_profiles WHERE user_id = ?`,
      [userId]
    );
    const profile = (profileRows as any[])[0];
    if (profile?.weight_kg) {
      const seedDate = (profile.updated_at as Date)
        .toISOString()
        .split("T")[0];
 ...
 */
export async function computeStreakAndStats(userId: number): Promise<DashboardStats> {
    ...
    for (const d of sortedDates) {
      ...
      if (prevDate !== null) {
        const diff = Math.round(
          (prevDate.getTime() - curr.getTime()) / (1000 * 60 * 60 * 24)
        );
        if (diff === 1) {
          runningStreak++;
        } else {
  ...
  const weeklyRows = await dbAll(
    `SELECT COUNT(DISTINCT date) AS cnt FROM (
       SELECT date FROM workout_plans
  ...
  await dbRun(
    ...
    [userId, totalWorkouts, currentStreak, longestStreak, lastWorkoutDate]
  );
...
export async function getStrengthProgress(
  ...
  const topExercises = await dbAll(
  ...
  const history = await dbAll(
    `SELECT exercise_name, date, AVG(weight_used) AS weight_kg
     FROM workout_logs
  ...
  // Group by exercise
  const grouped = new Map<string, { date: string; weight_kg: number }[]>();
  for (const row of history) {
    const dateStr =
      row.date instanceof Date ? row.date.toISOString().split("T")[0] : String(row.date);
    if (!grouped.has(row.exercise_name)) grouped.set(row.exercise_name, []);
 ...
 */
export async function getMostImproved(userId: number): Promise<MostImproved | null> {
  const rows = await dbAll(
...
export async function getRecentWorkouts(
  userId: number,
  limit = 7
): Promise<{ date: string; focus: string }[]> {
  const rows = await dbAll(
    `SELECT date, focus FROM workout_plans
     WHERE user_id = ?
     ORDER BY date DESC
     LIMIT ?`,
    [userId, limit]
...
// ─────────────────────────────────────────────────────────────────────────────
// Body-metrics log (for POST /api/progress/metrics)
// ─────────────────────────────────────────────────────────────────────────────
 ...
 */
export function buildChatInsight(data: DashboardData): string {
  const lines: string[] = [];

  // Streak line
  if (data.stats.current_streak > 0) {
    lines.push(
      `🔥 You're on a **${data.stats.current_streak}-day workout streak** — don't break it!`
    );
  } else {
    lines.push(
      `💪 You've completed **${data.stats.total_workouts} workout${data.stats.total_workouts !== 1 ? "s" : ""}** in total.`
    );
  }
  ...
  if (data.stats.weekly_workouts > 0) {
    lines.push(
      `📅 This week you've trained **${data.stats.weekly_workouts} day${data.stats.weekly_workouts !== 1 ? "s" : ""}** (longest streak ever: ${data.stats.longest_streak}).`
    );
  }
  ...
  if (data.most_improved) {
    ...
    lines.push(
      `📈 Your **${exercise}** improved by **${improvement_pct}%** — that's real strength progress!`
    );
  } else if (data.strength_progress.length > 0) {
    lines.push(
      `💪 You've been tracking **${data.strength_progress.length} exercise${data.strength_progress.length !== 1 ? "s" : ""}** — keep logging to unlock strength insights!`
    );
  }
