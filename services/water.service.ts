 ...
 */
export async function addWaterIntake(userId: number, amount: number, source: 'manual' | 'ai' | 'system' = 'manual') {
  const date = new Date().toISOString().split('T')[0];
  
  // 1. Insert log
  await pool.execute(
    `INSERT INTO water_logs (user_id, intake_amount, source) VALUES (?, ?, ?)`,
    [userId, amount, source]
  );

  await pool.execute(
    `INSERT INTO daily_hydration_progress (user_id, date, total_consumed)
     VALUES (?, ?, ?)
  ...
  const [progRows] = await pool.execute(
    "SELECT completion_percentage FROM daily_hydration_progress WHERE user_id = ? AND date = ?",
    [userId, date]
  );
  const pct = (progRows as any[])[0]?.completion_percentage || 0;
...
export async function updateWaterIntake(userId: number, logId: number, amount: number) {
  const [rows] = await pool.execute(
    "SELECT intake_amount FROM water_logs WHERE id = ? AND user_id = ?",
    [logId, userId]
  );
  const oldAmount = (rows as any[])[0]?.intake_amount || 0;
  const diff = amount - oldAmount;

  await pool.execute(
    "UPDATE water_logs SET intake_amount = ? WHERE id = ? AND user_id = ?",
    [amount, logId, userId]
  );

  const date = new Date().toISOString().split('T')[0];
  await pool.execute(
    `UPDATE daily_hydration_progress 
...
export async function deleteWaterIntake(userId: number, logId: number) {
  ...
  const amount = (rows as any[])[0]?.intake_amount || 0;
...
export async function setHydrationGoal(userId: number, goalMl: number, isAI = false, reason?: string) {
  ...
  // 1. Save goal history
  await pool.execute(
    `INSERT INTO water_goals (user_id, daily_goal, generated_by_ai, goal_reason)
     VALUES (?, ?, ?, ?)`,
    [userId, goalMl, isAI ? 1 : 0, reason || null]
  );

  // 2. Update today's progress goal
  await pool.execute(
    `INSERT INTO daily_hydration_progress (user_id, date, daily_goal)
     VALUES (?, ?, ?)
     ON DUPLICATE KEY UPDATE 
...
export async function getTodayHydration(userId: number) {
  const date = new Date().toISOString().split('T')[0];
  const [rows] = await pool.execute(
    "SELECT * FROM daily_hydration_progress WHERE user_id = ? AND date = ?",
    [userId, date]
  );
  return (rows as any[])[0] || { user_id: userId, date, total_consumed: 0, daily_goal: 2000, completion_percentage: 0 };
}
...
export async function getHydrationHistory(userId: number, limit = 7) {
  const [rows] = await pool.execute(
    `SELECT * FROM daily_hydration_progress 
     WHERE user_id = ? 
