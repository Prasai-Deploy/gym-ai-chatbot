 ...
 */
export async function updateWeeklyProgress(userId: number, date: string, data: ProgressUpdate) {
  const sql = `
 ...
 */
export async function updateDailyFitnessStats(userId: number, data: Partial<{ daily_progress_percentage: number, active_minutes: number, completed_goals: number }>) {
  const sql = `
 ...
 */
export async function getWeeklyChartData(userId: number) {
  const [rows] = await pool.execute(
    `SELECT date, workouts_completed, exercises_completed, calories_burned, 
 ...
 */
export async function getDailyStats(userId: number) {
  const [rows] = await pool.execute(
    `SELECT * FROM daily_fitness_stats WHERE user_id = ?`,
    [userId]
  );
  return (rows as any[])[0] || { daily_progress_percentage: 0, active_minutes: 0, completed_goals: 0 };
}

/**
 * Logs manual user progress entry.
 */
export async function logManualProgress(userId: number, data: any) {
  ...
  await pool.execute(
    `INSERT INTO user_progress (user_id, date, calories_consumed, calories_burned, water_ml)
     VALUES (?, ?, ?, ?, ?)
     ON DUPLICATE KEY UPDATE
       calories_consumed = calories_consumed + VALUES(calories_consumed),
       calories_burned = calories_burned + VALUES(calories_burned),
       water_ml = water_ml + VALUES(water_ml)`,
    [userId, date, 0, data.calories || 0, data.water || 0]
  );
 ...
 */
export async function buildProgressInsight(userId: number) {
