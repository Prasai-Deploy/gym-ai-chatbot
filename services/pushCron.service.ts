...
export async function runHourlyPushTasks(): Promise<void> {
  const now = new Date();
  ...
  const reminderUsers = await dbAll(
    `SELECT ns.user_id
  ...
  for (const row of reminderUsers) {
    await sendPushToUser(
      row.user_id,
      "Time to move! 💪",
      "Your AI coach is waiting. Log a workout and keep your streak alive.",
      "/"
    );
  }
  ...
  if (now.getHours() === 19) {
    const atRiskUsers = await dbAll(
      `SELECT ns.user_id, us.current_streak
       FROM user_notification_settings ns
       JOIN push_subscriptions ps ON ps.user_id = ns.user_id
       JOIN user_streaks us ON us.user_id = ns.user_id
       WHERE ns.streak_alerts = 1
         AND us.current_streak > 0
         AND (us.last_active_date IS NULL OR us.last_active_date < ?)`,
      [today]
    );
    for (const row of atRiskUsers) {
      await sendPushToUser(
        ...
        `You have a ${row.current_streak}-day streak. Log something today to keep it going!`,
        "/"
      );
    }
  }
...
export async function runWeeklySummaryIfDue(): Promise<void> {
  const now = new Date();
  if (now.getDay() !== 0 || now.getHours() !== 9) return; // Sunday 9 AM only

  const subs = await dbAll(
    `SELECT ns.user_id FROM user_notification_settings ns
     JOIN push_subscriptions ps ON ps.user_id = ns.user_id
  ...
  for (const row of subs) {
    await sendPushToUser(
      ...
      "Check out how you crushed it this week — view your progress now.",
      "/progress"  // deeplinks to the progress dashboard tab
    );
  }
}
...
export async function notifyBadgeEarned(
  ...
  const [settingsRow] = await pool.execute(
    "SELECT badge_alerts FROM user_notification_settings WHERE user_id = ?",
    [userId]
  );
  const settings = (settingsRow as any[])[0];
  if (!settings?.badge_alerts) return;

  await sendPushToUser(
    userId,
    `🏅 Badge Unlocked: ${badgeName}!`,
    "You just earned a new achievement. Keep it up!",
    "/badges"
  );
}
