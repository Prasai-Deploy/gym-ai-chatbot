...
CREATE TRIGGER update_nutrition_logs_modtime BEFORE UPDATE ON public.nutrition_logs FOR EACH ROW EXECUTE FUNCTION update_modified_column();
...
CREATE POLICY "Members manage own nutrition" ON public.nutrition_logs FOR ALL USING (auth.uid() = user_id);
