...
router.post("/award", async (req, res) => {
    ...
    try {
        ...
        if (trigger === "first_chat") {
            ...
            const [countRow] = await pool.execute("SELECT COUNT(*) AS total FROM workout_logs WHERE user_id = ?", [uid]);
            const count = countRow[0]?.total ?? 0;
            triggers.firstWorkout = count <= 1;
            triggers.workoutCount = count;
            // Check early bird
            const hour = new Date().getHours();
            if (hour < 8)
                triggers.earlyBird = true;
        }
            ...
            const rows = (await pool.execute(`SELECT date FROM nutrition_logs WHERE user_id = ? ORDER BY date DESC LIMIT 7`, [uid]))[0];
            let consecutive = 0;
